...
public class ThirdPersonOrbitCamBasicMobile : NetworkBehaviour 
{
	public bool isMyOwner = false;
	...
	{
		//if (!IsOwner) return;
		//await MatchMakerClient.instance.isPlayerSpawned; 
	...
	private int NonAnalogScreenMove()
	{
		...
		{
			if (!analogIds.Contains(ControlFreak2.CF2Input.GetTouch(i).fingerId.ToString()) &&
				ControlFreak2.CF2Input.GetTouch(i).phase == TouchPhase.Moved)
			{
				return i;
			}
		}
	...
	void Update()
	{
		...
		Debug.Log("Touch Index "+touchIndex);
		...
		if (camTouchscreen &&  touchIndex >= 0)
		{
			...
			Debug.Log("Inside tochindex");
			angleH += (ControlFreak2.CF2Input.GetTouch(touchIndex).deltaPosition.x / 10f) * horizontalAimingSpeed;
			angleV += (ControlFreak2.CF2Input.GetTouch(touchIndex).deltaPosition.y / 10f) * verticalAimingSpeed;
		}
